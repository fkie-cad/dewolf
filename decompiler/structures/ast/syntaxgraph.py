...
from decompiler.structures.ast.ast_node_factory import ASTNodeFactory
...
from decompiler.structures.ast.reachability_graph import ReachabilityGraph, SiblingReachability
...
class AbstractSyntaxInterface(ABC):
    ...
    def _remove_node(self, node: AbstractSyntaxTreeNode):
        ...
        if isinstance(node.parent, SeqNode):
            node.parent._sorted_children = tuple(child for child in node.parent._sorted_children if hash(child) != hash(node))
        if isinstance(node, CodeNode):
    ...
    def _substitute_node(self, replacee: AbstractSyntaxTreeNode, replacement: AbstractSyntaxTreeNode):
        ...
        if isinstance(parent, SeqNode):
            parent._sorted_children = tuple(replacement if hash(child) == hash(replacee) else child for child in parent._sorted_children)
    ...
    def _replace_subtree(self, replacee_root: AbstractSyntaxTreeNode, replacement_root: AbstractSyntaxTreeNode):
        ...
        if isinstance(parent, SeqNode):
            parent._sorted_children = tuple(replacement_root if hash(nd) == hash(replacee_root) else nd for nd in parent._sorted_children)
    ...
    def _add_sequence_node_before(self, node: AbstractSyntaxTreeNode) -> SeqNode:
        ...
        if parent is not None:
            if isinstance(parent, SeqNode):
                parent._sorted_children = tuple(new_seq if hash(child) == hash(node) else child for child in parent._sorted_children)
