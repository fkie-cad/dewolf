        ...
        if isinstance(expr.type, ArrayType):
            match expr.type.type:
                ...
                case Integer(size=8, signed=False):
                    ...
                    return f'"{val}"' if len(val) <= MAX_GLOBAL_INIT_LENGTH else f'"{val[:MAX_GLOBAL_INIT_LENGTH]}..."'
                case Integer(8):
                    val = "".join(expr.value[:MAX_GLOBAL_INIT_LENGTH])
                    return f'"{val}"' if len(val) <= MAX_GLOBAL_INIT_LENGTH else f'"{val[:MAX_GLOBAL_INIT_LENGTH]}..."'
                case _:
                    return f'{", ".join([self.visit_constant(expressions.Constant(x, expr.type.type)) for x in expr.value])}'  # Todo: Should we print every member? Could get pretty big
