...
* ``test_ghidra_smoke`` -- a small curated set of functions, run on every PR (fast: it still pays the
  JVM startup, so we keep it short).
...
The whole module is skipped when Ghidra/pyghidra is unavailable, so it is a no-op in the Binary
Ninja CI image and on developer machines without a Ghidra install.
"""
...
def test_ghidra_smoke(sample, function_name):
    ...
    ok, detail = run_decompilation(sample, function_name, frontend="ghidra")
    if not ok:
