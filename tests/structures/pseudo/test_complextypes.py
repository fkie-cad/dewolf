...
from decompiler.structures.pseudo.complextypes import ComplexTypeMember, ComplexTypeSpecifier, Enum, Struct, Union
...
class TestStruct:
    ...
    def test_copy(self, book: Struct):
        ...
        assert id(new_book.get_member_by_offset(0)) != id(
            book.get_member_by_offset(0))
    ...
    @pytest.fixture
    def book(self) -> Struct:
        return Struct(name="Book", members={0: ComplexTypeMember(size=32, name="title", offset=0, type=Pointer(Integer.char())),
                                            4: ComplexTypeMember(size=32, name="num_pages", offset=4, type=Integer.int32_t()),
                                            8: ComplexTypeMember(size=32, name="author", offset=8, type=Pointer(Integer.char()))}, size=96)

    @pytest.fixture
    def title(self) -> ComplexTypeMember:
        return ComplexTypeMember(size=32, name="title", offset=0, type=Pointer(Integer.char()))

    @pytest.fixture
    def num_pages(self) -> ComplexTypeMember:
        return ComplexTypeMember(size=32, name="num_pages", offset=4, type=Integer.int32_t())

    @pytest.fixture
    def author(self) -> ComplexTypeMember:
        return ComplexTypeMember(size=32, name="author", offset=8, type=Pointer(Integer.char()))


@pytest.fixture
def record_id() -> Union:
    return Union(name="RecordID", size=64, members=[ComplexTypeMember(size=32, name="float_id", offset=0, type=Float.float()),
                                                    ComplexTypeMember(size=32, name="int_id", offset=0, type=Integer.int32_t()),
                                                    ComplexTypeMember(size=Float.double().size, name="double_id", offset=0,
                                                                      type=Float.double())
                                                    ])
...
class TestEnum:
    ...
    @pytest.fixture()
    def color(self):
        return Enum(0, "Color", {0: ComplexTypeMember(0, "red", value=0, offset=0, type=Integer.int32_t()),
