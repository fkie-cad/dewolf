...
@pytest.mark.parametrize(
    ...
    [
        (o := Variable("v", _i32, 0), r := Variable("x", _i32, 1), SubstituteVisitor.identity(o, r)),
        (o := Variable("v", _i32, 0), r := Variable("x", _i32, 1), SubstituteVisitor.equality(o, r)),
        (o := Variable("v", _i32, 0), o, SubstituteVisitor.identity(Variable("v", _i32, 0), Variable("x", _i32, 1))),
        (o := Variable("v", _i32, 0), r := Variable("x", _i32, 1), SubstituteVisitor.equality(Variable("v", _i32, 0), r)),
        (Assignment(a := Variable("a"), b := Variable("b")), Assignment(a, c := Variable("c")), SubstituteVisitor.identity(b, c)),
        (Assignment(a := Variable("a"), b := Variable("b")), Assignment(c := Variable("c"), b), SubstituteVisitor.identity(a, c)),
        (
        ...
        (RegisterPair(a := Variable("a"), b := Variable("b")), RegisterPair(a, c := Variable("c")), SubstituteVisitor.identity(b, c)),
        (Call(f := Variable("f"), [a := Variable("a")]), Call(f, [b := Variable("b")]), SubstituteVisitor.identity(a, b)),
        (Call(f := Variable("f"), [a := Variable("a")]), Call(g := Variable("g"), [a]), SubstituteVisitor.identity(f, g)),
        (
            Phi(
                a3 := Variable("a", _i32, 3),
                [a2 := Variable("a", _i32, 2), a1 := Variable("a", _i32, 1)],
                {
                    BasicBlock(2): a2,
                    BasicBlock(1): a1,
                },
        ...
        (Return([a := Variable("a")]), Return([b := Variable("b")]), SubstituteVisitor.identity(a, b)),
    ],
)
