...
import pytest
from dewolf.pipeline.dataflowanalysis.deadcodeelimination import DeadCodeElimination
...
"""Local (same basicblock) tests."""


def test_empty(task: MockDecompilerTask):
    """DCE should not crash facing an empty cfg."""
    DeadCodeElimination().run(task)


def test_assignment_unnessecary(task: MockDecompilerTask):
...
def test_assignment_nessecary_for_call(task: MockDecompilerTask):
    ...
    +-----------------+
    """
    instructions = [Assignment(variable(), Constant(1)), Assignment(variable(version=1), Call(FunctionSymbol("func", 0x42), [variable()]))]
...
def test_assignment_nessecary_for_branch(task: MockDecompilerTask):
...
def test_assignment_nessecary_for_return(task: MockDecompilerTask):
...
def test_phi_unnessecary(task: MockDecompilerTask):
...
def test_phi_nessecary(task: MockDecompilerTask):
...
def test_loop(task: MockDecompilerTask):
...
def test_circular_dependency(task: MockDecompilerTask):
...
def test_dereference_used(task: MockDecompilerTask):
    """
    +----------------------------+
    |        *(a#0) = b#1        |
    |      a#1 = a#0 + 0x2       |
    ...
    instructions = [
        ...
        Assignment(
            variable("b", 2),
            Call(FunctionSymbol("func", 0x42), [BinaryOperation(OperationType.plus, [variable("a", 2), Constant(0x4)]), variable("b", 1)]),
        ),
    ]
...
def test_dead_variables_in_return_values():
...
def test_dead_variables_with_ambiguous_type():
    ...
    cfg.add_nodes_from(
        [
            ...
            branch_true := BasicBlock(
                1,
                instructions=[Assignment(Variable("p", ssa_label=0), UnaryOperation(OperationType.address, [Variable("a", ssa_label=1)]))],
            ),
            branch_false := BasicBlock(
                2,
                instructions=[Assignment(Variable("p", ssa_label=1), UnaryOperation(OperationType.address, [Variable("b", ssa_label=7)]))],
            ),
            end := BasicBlock(
                3,
                instructions=[
...
def test_never_remove_relations_basic():
    """
...
def test_never_remove_relations():
    """
    test_memory test14
    +----------------------------+  +----------------------------+
    |             0.             |  |             0.             |
