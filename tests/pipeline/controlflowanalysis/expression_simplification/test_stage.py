...
@pytest.mark.parametrize(
    ...
    [
        (
            [TermOrder()],
            Assignment(_v_i32("a"), _add(_c_i32(1), _v_i32("b"))),
            Assignment(_v_i32("a"), _add(_v_i32("b"), _c_i32(1)))
        ),
        (
            [CollapseConstants()],
            Assignment(_v_i32("a"), _sub(_c_i32(10), _add(_c_i32(3), _c_i32(2)))),
...
@pytest.mark.parametrize(
    ...
    [
        (
            [_RedundantChanges()],
            Assignment(Variable("a"), BinaryOperation(OperationType.plus, [Constant(1), Variable("b")])),
            10,
            True
        ),
        (
            [_NoChanges()],
            Assignment(_v_i32("a"), _v_i32("b")),
            0,
            False
        )
    ]
...
def test_simplify_instructions_with_rule_set_max_iterations(
        ...
        instruction: Instruction,
        max_iterations: int,
        expect_exceed_max_iterations: bool
):
